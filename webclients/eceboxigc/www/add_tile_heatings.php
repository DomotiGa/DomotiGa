...
$monUrl2 = split ("/", $monUrl2);
    ...
    if($count != 0){
        $reponse = $db->query('SELECT heating, counts FROM tiles WHERE ID = '.$_GET['id']);
    }
...
// Get list of heating for the enabled scenario
...
<?php
  foreach($list_scenarii AS $thermostat) {
		...
		echo "<tr> 
		<td >
		<input type=\"radio\" name=\"options[]\" value=\"".$thermostat['idthermo']."\">
		  </td> 	
...
if ($envoi == 'yes') 
{
        if($nbtuile < 3)
	{
		if($myheatings8 != $myheatings." ")
		{$nbtuile = $nbtuile + 1;}	
		$req =  $db->prepare('UPDATE tiles SET heating = (:theheatingtuile) WHERE  tiles.ID ='.$_GET['id']);
		$req->execute(array('theheatingtuile' => $myheatings8  ));
...
$nbtuile = $nbtuile - 1;
	$req =  $db->prepare('UPDATE tiles SET heating = (:theheatingtuile) WHERE  tiles.ID ='.$_GET['id']);
	$req->execute(array('theheatingtuile' => $myheatings7  ));
		$req =  $db->prepare('UPDATE tiles SET counts = (:nbtuile) WHERE  tiles.ID ='.$_GET['id']);
		$req->execute(array('nbtuile' => $nbtuile  ));
}
...
<br>
<h3>Added Heatings</h3>
<?
if( $myheatings == "")
{

echo " <h3 style=\"text-align:center;\"> You have not added any Heatings yet. </h3>";
} else 
{
...
$u="debug";
		echo "<tr> 
<td >
		<input type=\"radio\" name=\"options2[]\" value=\"".$thermostat['idthermo']."\">
		  </td> 	
		<td> ".$thermostat['namethermo']." </td> 
		<td> ".$thermostat['descriptionthermo']." </td>
		<td>
...
<td >
		...
		if ($u != "debug")
		{
			...
			$myheatings = "";
				$req =  $db->prepare('UPDATE tiles SET heating = (:theheatingtuile) WHERE  tiles.ID ='.$_GET['id']);
        $req->execute(array('theheatingtuile' => $myheatings  ));
		}
